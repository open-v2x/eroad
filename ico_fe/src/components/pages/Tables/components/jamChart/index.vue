...
import { road_intersection, road_yongdu_tian, road_yongdu_duan } from '../../api'
      ...
      if (data.timeQuantumActive) {
        road_yongdu_duan({
          ...
          if (res.code === 200) {
            ...
            this.transit.series[0].data = res.data.map(item => item.delay_time_prop) // 饱和度
            this.transit.series[1].data = res.data.map(item => item.tra_time_index) // 效率
          }
        })
      } else {
        road_yongdu_tian({
