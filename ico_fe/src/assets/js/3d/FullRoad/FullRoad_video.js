...
export default class FullRoad_video{
    socket = null;
    ...
    constructor(viewer){
        ...
        this.ws.onmessage(msg=>{
            let data = JSON.parse(msg);
            let targets = data.Info;
            let deviceId = '123';
            if(targets.length>0){
                targets.forEach(target =>{//循环待优化
                    let carId = target.trackid;
                    ...
                    let lng = Number(target.location.longitude);
                    let lat = Number(target.location.latitude);
                    let position = Cesium.Cartesian3.fromDegrees(lng,lat,this.h);//(lng+0.00006,lat+0.000015,0.4);
                    let hpRoll =  new Cesium.HeadingPitchRoll(- 1.5707964,0,0);// - 1.5707964
                    if(!getCar){//如果是新车
